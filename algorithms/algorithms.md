...
- Такая очередь поддерживает (как минимум) две операции вставка и извлечение максимального (приоритетного), 
подходит для задач где нужно не по порядку брать, а в зависиости от чего-то, например распределение задач по процессорам, 
брать сначала долгие задачи.
- Такую очередь удобно строить на [бинарнной куче](https://github.com/dmboytsov/u4i-mat-4ast/blob/main/algorithms/data-structures.md#%D0%B1%D0%B8%D0%BD%D0%B0%D1%80%D0%BD%D0%B0%D1%8F-%D0%BA%D1%83%D1%87%D0%B0-binary-heap) при добавление в кучу элемент встает на свое место в дереве, 
вставка за логарифмическое время, извлечение максимального - это взять корень, и перестроить дерево (выбрать кто из потомков теперь корень)
