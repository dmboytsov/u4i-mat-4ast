...
## Интерфейс
- Интерфейс в Go - это тип, который определяет набор методов. Любой тип, реализующий все методы интерфейса, автоматически удовлетворяет этому интерфейсу (утиная типизация).
...
- any - алиас для пустого интерфейса c 1.18
```go
var value any
value = 5 // int
value = "Go" // string
...
```
## Incomparable types
- типы коткорые нельзя сравнить через `==`  `!=` это slice, map, функции, структуры, которые содержат incomparable types.
- Мапы и слайсы можно сравнить через - `reflect.DeepEqual()`
- не сравниваемые потому, что не ясно как сравнивать, по содержимому дорого
- Интерфейсы можно сравнивать, но если они содержат несравнимые значения, это вызовет panic
- А **каналы можно сравнивать** каналы равны, если это переменные на один и тот же канал
...
```
- указатель на элемент мапы, код не скомпелируется, указатель может начать показывать на мусор (при перестроение мапы, при ее росте)
```go
valAddr := &m[key] // ошибка компиляции
```
...
- если этачить метод к указателю на структуру, то изменения внутри метода видны снаружи. Если этачить к значению, то изменения не применятся (сетер не будет работать)
